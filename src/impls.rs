...
use crate::{Cmd, CommandBuilder, Error, OutputResult, ProcessResult, Vec8ToString};
...
impl OutputResult for Output {
    fn to_result(&self) -> crate::Result<Vec<u8>> {
        if self.status.success() && self.stderr.is_empty() {
            ...
            Err(Error::CmdError(CmdError::from_err(
                self.status,
...
impl Cmd {
                    ...
                    if !killed {
                        if let Some(timeout) = timeout {
                            if now.elapsed() > timeout {
                                warn!(
                                    "timeout passed `{}ms`... kill the process",
    ...
    fn read_to_end(
        ...
        if let Some(stdout) = stdout {
            ...
            for line in <BufReader<ChildStdout> as BufReaderExt<BufReader<ChildStdout>>>::lines_vec(
                stdout_reader,
            ) {
        ...
        if let Some(stderr) = stderr {
            ...
            for line in <BufReader<ChildStderr> as BufReaderExt<BufReader<ChildStderr>>>::lines_vec(
                stderr_reader,
            ) {
